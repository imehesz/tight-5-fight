...
func _ready() -> void:
	...
	add_title(box, "GAME OVER", 36, Color(0.95, 0.3, 0.25))
	...
	add_spacer(box, 8)
	add_title(box, "FINAL SCORE: %d" % GameState.score, 22, Color(0.9, 0.9, 0.95))
	if GameState.last_run_rank >= 0:
		add_title(box, "NEW HIGH SCORE — #%d!" % (GameState.last_run_rank + 1), 16)
	add_spacer(box, 14)
